...
function generateCelestialTextureCanvasesLocal(descriptor, textureSize) {
  ...
  const roughnessCanvas =
    ...
    makeFlatMapCanvas(
      textureSize,
      textureSize,
      flattenAuxMaps ? [112, 112, 112, 255] : [180, 180, 180, 255],
    );
...
function applyDescriptorMapsToRuntime(runtime, descriptor, signature, entry) {
  ...
  if (descriptor.bodyType === "gasGiant") {
    ...
    runtime.body.material.metalness = 0.03;
    if (runtime.body.material.normalScale?.set) runtime.body.material.normalScale.set(0.4, 0.4);
    if ("clearcoat" in runtime.body.material) runtime.body.material.clearcoat = 0.02;
    if ("clearcoatRoughness" in runtime.body.material)
      runtime.body.material.clearcoatRoughness = 0.36;
  ...
  // Pull the camera back when rings are visible so the ring system fits
  // within the circular canvas clip and is clearly distinct from the body.
  runtime.camera.position.z = showRing ? 6.0 : 3.5;
  runtime.camera.updateProjectionMatrix();
...
async function doRecipeSnapshot(targetCanvas, model) {
  ...
  const spin = hashUnit(descriptor.seed || "recipe") * Math.PI * 2;
