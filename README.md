...
- Star modelling with metallicity, advanced R/L/T overrides, and stellar evolution.
- Planetary system generation with habitable zone, frost line, orbit-slot assignment, and system poster view.
- Rocky planets, gas giants, and moons with physics-driven derived outputs.
- Unified body rendering pipeline across Planet, System Poster, Visualiser, and Apparent views.
- Auto-ring detection and style sync for gas giants based on ring optical-depth class.
...
- `npm run backup:live` - Create a zip backup of live deploy files in `Backup/` and keep the newest 3 (PowerShell required).
...
- Three.js modules for the splash overlay 3D planet.
- KaTeX for formula rendering on the Science and Maths page.
...
- `references/` - Validation and source reference materials.
- `projects-showcase/` - Standalone project showcase microsite (separate from core app runtime).
